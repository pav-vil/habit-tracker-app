...
def dashboard():
    ...
    try:
        ...
        if not user_settings:
            user_settings = PeriodSettings(user_id=current_user.id, period_tracking_enabled=False)
            db.session.add(user_settings)
            db.session.commit()

        if not user_settings.period_tracking_enabled:
            flash('Please enable period tracking in settings first.', 'info')
            return redirect(url_for('periods.settings'))
        ...
        # Get current phase info (may be None if no cycles yet)
        phase_info = calculate_current_phase(current_user) if past_cycles else None

        # Generate predictions if we have at least one past cycle
        if len(past_cycles) > 0 and len(predicted_cycles) < 3:
            try:
        ...
        flash(f'Error loading period tracker: {str(e)}', 'danger')
...
def add_cycle():
    ...
    if form.validate_on_submit():
        try:
            ...
            # Check for overlapping cycles (simplified - just check if start date already exists)
            ...
            if existing:
                flash('A period with this start date already exists.', 'danger')
                return render_template('periods/add_cycle.html', form=form)
            ...
            db.session.rollback()
            flash(f'Error adding period cycle: {str(e)}', 'danger')
            return render_template('periods/add_cycle.html', form=form)
